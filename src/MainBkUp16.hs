...
decompress :: IO ()
decompress = do
  input <- Bs.readFile "serialized.bin"--"Bee 52 (U).nes"
  let
    binaryTail = Bs.drop  0 input --0x47E input--0x67e9 input --
...
deserializeTiles = map deserializeTile . chunksOf 16 . Bs.unpack
  where
    deserializeTile :: [Word8] -> Tile
    deserializeTile binTile = chunksOf 8 $ zipWith formColor (take 64 planesBits) (drop 64 planesBits)
...
buildLuts noRleRows = map buildLut [0..3] --for each possible color
  where
    ...
    buildLut number = Lut ((fromIntegral.length) nexts) $ sortMode3 nexts
      where
        nexts = [next | (cur, next) <- sortedTables, cur == number]--filter for appropriate color
        sortMode3 (a:b:c) = a: sort (b:c) --in mode 3, b has min color and c - max despite of frequency
        sortMode3 xs = xs
...
putTile luts prev tile = zipWithM_ putRow (prev: tile) tile
  where
    ...
    putRow prevRow curRow = if curRow == prevRow
      then putWord8 1 1 -- put RLE row flag
        ...
        zipWithM_ (putPixel luts) curRow $ tail curRow
...
compress = do
  ...
  let
    ...
    serialized = P.runPut (P.putWord8 (fromIntegral lastTile) >> --put size
                   runBitPut (mapM_ serializeBlock tileBlocks)) --then serialized blocks
