...
parseCommand  = do
  size <- getWord8 8
  tail <$> iterateNTimesM 3 parseBlock ([], [[]])--empty lut and result tiles
    where
      parseBlock :: (Luts, [[Color]]) -> BitGet (Luts, [[Color]])
      parseBlock (prevLuts, _) = do
        skipLutsUpdateFlag <- getBool
        if skipLutsUpdateFlag
          then do
            tile1 <- parseTile prevLuts
            return (prevLuts, tile1)
          else do
            currentLuts <- parseLuts
            tile2 <- parseTile currentLuts
...
parseTile luts = tail <$> iterateNTimesM 9 parseRow [] --empty prev row
    ...
    parseRow prevRow = do
      ...
      if repeatRowFlag
        then return prevRow
          ...
          firstCol <- getWord8 2
          iterateNTimesM 8 parsePixel firstCol
